...
_CSLIST_ () {	# create checksum file RDR/.conf/sha512.sum and compare with RDR/sha512.sum 
	FAUTH="DRLIM GAUTH LIBAUTH QUIET"	# exemption file list
...
(git submodule update --recursive --remote "$IMFSTRG" || git submodule add "$MRASTRGz" "$IMFSTRG") && cd "$RDR/$IMFSTRG" && _PRCS_ && cd "$RDR" || _PESTRG_	# the command ` git submodule help ` and the book https://git-scm.com/book/en/v2/Git-Tools-Submodules have more information about git submodules
...
(git submodule update --recursive --remote "$IMFSTRG" || git submodule add "$MRASTRG" "$IMFSTRG") && cd "$RDR/$IMFSTRG" && _PRCS_ && cd "$RDR" || _PESTRG_	# the command ` git submodule help ` and the book https://git-scm.com/book/en/v2/Git-Tools-Submodules have more information about git submodules
...
(git submodule update --recursive --remote "$IMFSTRG" || git submodule add "$MRASTRG" "$IMFSTRG") && cd "$RDR/$IMFSTRG" && _PRCS_ && cd "$RDR" || _PESTRG_ 
