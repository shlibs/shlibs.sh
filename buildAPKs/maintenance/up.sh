...
sleep 0.42 # added for network latency support on fast networks; See ` grep -hC 4 -r ~/buildAPKs/scripts ` for more latency applications when BuildAPKs is installed
git submodule update --init --recursive --remote scripts/bash/shlibs # ` git submodule help ` has more information
sleep 0.42 # added network latency support on fast networksnetworks; See ` grep -hC 4 -r ~/buildAPKs/scripts ` for more latency applications when BuildAPKs is setup
